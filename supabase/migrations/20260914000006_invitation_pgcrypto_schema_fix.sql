-- Supabase installs pgcrypto in the extensions schema, outside the SECURITY DEFINER search path.
...
begin
  ...
  from public.company_invitations invitation join public.company_members member on member.id = invitation.member_id join public.companies company on company.id = invitation.company_id
  where invitation.token_hash = encode(extensions.digest(convert_to(p_token, 'UTF8'), 'sha256'::text), 'hex')

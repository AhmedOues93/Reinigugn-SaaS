...
export function InvitationSignUp() {
  ...
  useEffect(() => { if (state.status === 'success' && state.id === 'accepted') router.push('/dashboard'); }, [router, state]);
...
export function InvitationAcceptButton() {
  ...
  const router = useRouter();
  useEffect(() => { if (state.status === 'success') router.push('/dashboard'); }, [router, state]);

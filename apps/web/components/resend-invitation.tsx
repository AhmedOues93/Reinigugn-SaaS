...
export function ResendInvitation({ memberId }: { memberId: string }) {
  ...
  return <div className="space-y-3"><Button type="button" variant="outline" disabled={pending} onClick={resend}>{pending ? 'Wird erneuert ...' : 'Einladung erneut senden'}</Button>{message && <p className="text-sm text-primary">{message}</p>}{url && <a className="block break-all text-sm text-primary underline" href={url}>{url}</a>}</div>;
}

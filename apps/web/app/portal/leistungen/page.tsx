...
export default async function PortalServicesPage() {
  ...
  return (
    <>
      ...
      {records.length === 0 ? (
        <PortalEmptyState icon={<FileText className="size-5" />} title={t(locale, 'portal.services.empty')} />
      ) : (
            ...
            <li key={record.job_id}>
              ...
              >
                <div className="min-w-0 flex-1">
                  <p className="truncate font-medium text-slate-900">{record.object_name}</p>
                  <p className="mt-0.5 text-sm text-slate-600">{formatDate(locale, record.scheduled_date, 'long')}</p>

...
type Query = { week?: string; customer?: string; object?: string; employee?: string; status?: string };
...
export default async function PlanningPage({ searchParams }: { searchParams: Promise<Query> }) {
  ...
  return <div className="mx-auto max-w-7xl">
    <div className="mb-7 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between"><div><h1 className="text-2xl font-semibold">Planung</h1><p className="mt-2 text-slate-600">Wochenansicht aller geplanten Reinigungseinsaetze.</p></div><div className="flex gap-2"><Link href="/dashboard/planung/plaene"><Button variant="outline">Wiederkehrende Plaene</Button></Link><Link href="/dashboard/auftraege/neu"><Button><Plus className="mr-2 size-4" />Auftrag</Button></Link></div></div>
    <Card className="mb-5 p-4"><form className="grid gap-3 md:grid-cols-2 xl:grid-cols-5"><input type="hidden" name="week" value={dateKey(start)} /><select className="h-10 rounded-md border bg-white px-3 text-sm" name="customer" defaultValue={query.customer ?? ''}><option value="">Alle Kunden</option>{customers.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select><select className="h-10 rounded-md border bg-white px-3 text-sm" name="object" defaultValue={query.object ?? ''}><option value="">Alle Objekte</option>{objects.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select><select className="h-10 rounded-md border bg-white px-3 text-sm" name="employee" defaultValue={query.employee ?? ''}><option value="">Alle Mitarbeiter</option>{employees.map((item) => { const profile = Array.isArray(item.profiles) ? item.profiles[0] : item.profiles; return <option key={item.id} value={item.id}>{[profile?.first_name, profile?.last_name].filter(Boolean).join(' ')}</option>; })}</select><select className="h-10 rounded-md border bg-white px-3 text-sm" name="status" defaultValue={currentStatus}><option value="all">Alle Status</option><option value="PLANNED">Geplant</option><option value="CONFIRMED">Bestaetigt</option><option value="CANCELLED">Storniert</option></select><Button type="submit" variant="outline">Filtern</Button></form></Card>
    ...
    <div className="grid gap-3 lg:grid-cols-7">{days.map((day) => { const key = dateKey(day); const dayJobs = jobs.filter((job) => job.scheduled_date === key); return <Card key={key} className="min-h-44 p-3"><p className="text-sm font-semibold">{new Intl.DateTimeFormat('de-DE', { weekday: 'short', day: '2-digit', month: '2-digit' }).format(day)}</p><div className="mt-3 space-y-2">{dayJobs.length === 0 ? <p className="text-xs text-slate-400">Keine Einsaetze</p> : dayJobs.map((job) => { const object = Array.isArray(job.cleaning_objects) ? job.cleaning_objects[0] : job.cleaning_objects; const customer = Array.isArray(job.customers) ? job.customers[0] : job.customers; return <Link key={job.id} href={`/dashboard/auftraege/${job.id}`} className="block rounded border-l-2 border-teal-700 bg-slate-50 p-2 hover:bg-teal-50"><p className="text-xs font-medium">{formatJobTime(job.planned_start_at, job.planned_end_at)}</p><p className="mt-1 text-sm font-semibold">{object?.name || job.title}</p><p className="mt-1 text-xs text-slate-600">{customer?.name}</p><div className="mt-2"><JobStatusBadge status={job.status} /></div></Link>; })}</div></Card>; })}</div>
  </div>;
}

...
export async function createFollowUpJob(complaintId: string, _: FormState, formData: FormData): Promise<FormState> {
  ...
  if (!/^\d{4}-\d{2}-\d{2}$/.test(date) || !/^\d\d:\d\d$/.test(start) || !/^\d\d:\d\d$/.test(end) || end <= start) return failure('Bitte gib einen gueltigen Zeitraum fuer die Nacharbeit an.');
  try {
...
export async function createQualityInspection(_: FormState, formData: FormData): Promise<FormState> {
  ...
  try {
    ...
    if (error || !data) return failure('Die Qualitaetskontrolle konnte nicht gespeichert werden.');
    revalidateOperations(parsed.data.cleaning_object_id);
    return { status: 'success', id: data.id };
  } catch { return failure('Die Qualitaetskontrolle konnte nicht gespeichert werden.'); }
}
...
export async function uploadOperationalPhoto(scope: 'COMPLAINT' | 'QUALITY_INSPECTION', recordId: string, _: FormState, formData: FormData): Promise<FormState> {
  try {
    ...
    if (!(file instanceof File)) return failure('Bitte waehle ein Foto aus.');
    const fileError = validateJobPhotoFile(file); if (fileError) return failure(fileError);
    if (!['BEFORE', 'AFTER', 'DOCUMENTATION'].includes(category)) return failure('Bitte waehle eine gueltige Kategorie.');

...
export default async function ServiceRecordPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <div className="mx-auto max-w-5xl"><Link href={`/dashboard/auftraege/${id}`} className="mb-5 inline-flex items-center gap-2 text-sm text-slate-600"><ArrowLeft className="size-4" />Zurueck zum Auftrag</Link><ServiceRecordView record={record} /></div>;
}

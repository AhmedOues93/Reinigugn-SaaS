...
export async function createJob(_: FormState, formData: FormData): Promise<FormState> {
  ...
  try {
    ...
    if (conflicts.length && !parsed.data.confirm_conflicts) {
      ...
      return { status: 'error', message: 'Bitte bestätige die Konfliktwarnung, wenn du den Auftrag trotzdem speichern möchtest.', conflictWarning: `${names} ist zu diesem Zeitpunkt bereits einem anderen Auftrag zugeordnet.` };
    }
...
export async function updateJob(jobId: string, _: FormState, formData: FormData): Promise<FormState> {
  ...
  try {
    ...
    if (relevant.length && !parsed.data.confirm_conflicts) return { status: 'error', message: 'Bitte bestätige die Konfliktwarnung, wenn du den Auftrag trotzdem speichern möchtest.', conflictWarning: 'Mindestens ein Mitarbeiter ist zu diesem Zeitpunkt bereits einem anderen Auftrag zugeordnet.' };

...
export async function updateCompanySettings(_: FormState, formData: FormData): Promise<FormState> {
  ...
  if (name.length < 2 || name.length > 120) return { status: 'error', message: 'Bitte gib einen gueltigen Firmennamen ein.' };
  if (!['de', 'en', 'fr', 'ar', 'tr', 'ro', 'pl'].includes(language)) return { status: 'error', message: 'Bitte waehle eine gueltige Standardsprache.' };

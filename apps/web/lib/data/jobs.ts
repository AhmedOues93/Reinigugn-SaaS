...
export async function listServiceSchedules() {
  ...
  const { data, error } = await supabase
    ...
    .select('id, name, customer_id, cleaning_object_id, valid_from, valid_until, is_active, acceptance_policy, billing_mode, assignment_mode, customers(name), cleaning_objects(name), schedule_rules(weekday, planned_start_time, planned_end_time, is_active), service_schedule_assignments(member_id)')

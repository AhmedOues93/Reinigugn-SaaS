...
export async function listJobs({ from, to, customerId, objectId, memberId, status = 'all' }: { from?: string; to?: string; customerId?: string; objectId?: string; memberId?: string; status?: JobStatusFilter }) {
  ...
  let query = supabase
    .from('jobs')
    .select('id, title, scheduled_date, planned_start_at, planned_end_at, status, priority, service_schedule_id, customer_id, cleaning_object_id, customers(name), cleaning_objects(name, city), job_assignments(member_id, company_members(profiles(first_name, last_name)))')
...
export async function getServiceSchedule(id: string) {
  ...
  const { data, error } = await supabase
    .from('service_schedules')
    .select('id, customer_id, cleaning_object_id, name, description, valid_from, valid_until, timezone, is_active, customers(id, name), cleaning_objects(id, name), schedule_rules(id, weekday, planned_start_time, planned_end_time, is_active), service_schedule_assignments(member_id, company_members(profiles(first_name, last_name)))')
    .eq('company_id', company.id).eq('id', id).maybeSingle();
...
export async function getDashboardMetrics() {
  ...
  const [{ count: todayJobs }, { count: plannedToday }, { data: assignments }, { count: weekJobs }] = await Promise.all([
  ...
  return { todayJobs: todayJobs ?? 0, plannedToday: plannedToday ?? 0, employeesScheduled: new Set((assignments ?? []).map((assignment) => assignment.member_id)).size, weekJobs: weekJobs ?? 0 };
}

  ...
  const { data, error } = await supabase
    ...
    .select(
      `id, quote_number, status, title, intro, currency, net_total_cents, vat_total_cents, gross_total_cents,
       recurring_net_monthly_cents, billing_mode, acceptance_policy, sent_at, valid_until, accepted_at, accepted_by_name, accepted_signature_text, acceptance_note, accepted_via, declined_at, decline_reason, created_at,

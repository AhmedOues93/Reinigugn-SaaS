...
export async function listEmployees({ search, role = 'all', status = 'all' }: { search?: string; role?: RoleFilter; status?: MemberFilter }) {
  ...
  let query = supabase
    ...
    .select('id, profile_id, role, status, invited_email, invited_first_name, invited_last_name, invited_phone, invited_at, joined_at, disabled_at, profiles(first_name, last_name, phone), employee_details(employee_number, weekly_hours, employment_start_date, notes, is_active)')
...
export async function getEmployee(id: string) {
  ...
  const { data, error } = await supabase
    .from('company_members')
    .select('id, company_id, profile_id, role, status, invited_email, invited_first_name, invited_last_name, invited_phone, invited_at, joined_at, disabled_at, created_at, profiles(first_name, last_name, phone, avatar_url), employee_details(employee_number, weekly_hours, employment_start_date, notes, is_active), company_invitations(id, expires_at, accepted_at, revoked_at, created_at)')
    .eq('company_id', company.id)
    .eq('id', id)
    .in('role', ['OFFICE', 'EMPLOYEE'])
    .maybeSingle();
